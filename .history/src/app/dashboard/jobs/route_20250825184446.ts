...
export async function GET() {
  ...
  const rows = await db.collection("jobs").find({ userId: session.user.id }).sort({ createdAt: -1 }).limit(200).toArray();
...
export async function POST(req: Request) {
  ...
  if (!title || !company) return NextResponse.json({ error: "Missing title/company" }, { status: 400 });
