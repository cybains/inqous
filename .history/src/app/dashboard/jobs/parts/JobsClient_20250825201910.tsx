...
export default function JobsClient() {
  ...
  async function save(jobId: string) {
    ...
    // feed excludes saved by design — refresh the current page with same filters
    loadList();
  }
      ...
      <div className="rounded-2xl border bg-white p-4 shadow-sm">
        <div className="grid gap-3 sm:grid-cols-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() => { setQ(""); setCompany(""); setLocation(""); router.push("/dashboard/jobs?page=1"); }}
              className="hidden w-full rounded-lg border px-3 py-2 text-sm hover:bg-gray-50 sm:block"
            >
              Clear
            </button>
          </div>
          ...
          {jobs.map((job) => {
            ...
            return (
              ...
              >
                ...
                {/* METADATA CHIPS (compact, always visible) */}
                ...
                {expanded && (
                  <div className="mt-3 border-t pt-3 text-sm text-gray-900">
                    {job.description ? (
                      // Render plain text safely; if you know it's sanitized HTML, swap to dangerouslySetInnerHTML
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      ...
                      <button
                        onClick={async () => { await save(job._id); }}
                ...
                {/* COLLAPSED ACTION (Save button on the right) */}
                {!expanded && (
        ...
        {pages.map((p, i) => {
          ...
          return (
            <span key={p} className="flex items-center">
              {needsDots && <span className="px-2">…</span>}
              <button
