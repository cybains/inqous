// app/dashboard/cv/documents/page.tsx
import { getServerSession } from "next-auth";
import { authOptions } from "@/lib/auth";
import getMongoClient from "@/lib/mongo";

function Tick({ tick }: { tick: string }) {
  const map: Record<string,string> = { grey:"bg-gray-300", yellow:"bg-yellow-400", green:"bg-green-500", red:"bg-red-500" };
...
export default async function DocumentsPage() {
  ...
  const docs = userId ? await db.collection("documents").find({ userId }).sort({ createdAt: -1 }).toArray() : [];
      ...
      <div className="overflow-hidden rounded-2xl border bg-white shadow-sm">
        <table className="min-w-full divide-y divide-gray-200 text-sm">
          ...
          <tbody className="divide-y divide-gray-100">
            ...
            {docs.length===0 && (
              <tr><td className="px-4 py-6 text-gray-500" colSpan={5}>No documents yet — upload your CV from the header or Upload page.</td></tr>
            )}
          </tbody>
        </table>
      </div>
