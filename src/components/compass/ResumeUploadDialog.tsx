...
export default function ResumeUploadDialog() {
  ...
  useEffect(() => { if (!open) { setStep("idle"); setFile(null); } }, [open]);
  ...
  return (
    <>
      <button onClick={()=>setOpen(true)} className="rounded-lg border border-gray-300 px-4 py-2 text-sm text-gray-900 hover:bg-gray-100">
        <Upload className="mr-2 inline h-4 w-4" /> Upload resume
      </button>
        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/30 p-4">
          <div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-xl">
            ...
            <div className="mt-4 space-y-4">
              <div className="flex flex-col items-center justify-center rounded-xl border-2 border-dashed border-gray-300 p-8 text-center hover:border-gray-400"
                ...
                <input ref={inputRef} type="file" accept=".pdf,.doc,.docx" className="hidden" onChange={(e)=>{const f=e.target.files?.[0]; if (f) setFile(f);}} />
              </div>
              ...
              <div className="flex justify-end gap-2">
                ...
                <button onClick={handleUpload} disabled={!file || !fullName || step!=="idle"} className="rounded-lg bg-black px-4 py-2 text-sm font-medium text-white disabled:opacity-50">Upload</button>
              </div>
            </div>
          </div>
        </div>
